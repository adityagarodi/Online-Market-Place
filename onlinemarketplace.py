shoplist=['Pen','Pencile','Book','Text Book','Scale','Marker',
          'Eraser','Sharpener','Stapler','Glue','Scissors',
         'Paper','Folder', 'Clipboard']
...
print('\n')
print("\033[1m=========== Your Bill===========\033[0m".center(shutil.get_terminal_size().columns))
for item in cart:
    print(f"{item['quantity']:3} x {item['subtype']:20} {item['type']:20} {item['name']:20} @ ₹{item['price']:.2f} = ₹{item['total']:.2f}")
print("\033[1m=================================\033[0m".center(shutil.get_terminal_size().columns))
print(f"\033[1mTotal Amount Payable: ₹{total_amount:.2f}\033[0m".center(shutil.get_terminal_size().columns))
print(f"\033[1mThank you for shopping with us, {name}!\033[0m".center(shutil.get_terminal_size().columns))
